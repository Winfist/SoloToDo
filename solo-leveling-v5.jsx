...
function calcFormationBonus(shadowArmy) {
  if (!shadowArmy) return { dungeonBonus:0, xpBonus:0, goldBonus:0 };
  const deployed = (shadowArmy.shadows||[]).filter(s=>s.isDeployed);
...
function calcSuccessChance(dungeon, stats, stratKey, skillBonuses, modifier, formationBonus) {
  let chance = 28;
...
function DungeonBattle({ dungeon, playerStats, theme, onResult, onClose, skillBonuses, modifier, formationBonus }) {
  ...
  const chance=calcSuccessChance(dungeon,playerStats,strategy.key,skillBonuses,modifier,formationBonus);
  ...
  const finishAllFloors=(completedFloors,bossWon=undefined)=>{
    ...
    setEquipDrop(drop);
    setResult({won,xp:xpResult,gold:goldResult,drop,floorsCleared:completedFloors.length,totalFloors:dungeon.floors,goldBonus});
    setPhase("result");
  };
        ...
        <div style={{width:"100%",maxWidth:440,padding:"0 20px",animation:"slideUp 0.4s ease"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
            {STRATEGIES.map(s=>{
              ...
              return(
                <button key={s.key} onClick={()=>setStrategy(s)} style={{padding:"14px 12px",borderRadius:12,textAlign:"left",background:isActive?s.color+"14":"rgba(10,10,20,0.6)",border:`1px solid ${isActive?s.color+"66":"#1e2940"}`,color:isActive?s.color:"#64748b",transition:"all 0.22s"}}>
...
export default function App() {
  ...
  const computeXpGain=useCallback((quest,streakBonus,equipBonuses,skillBonuses,penaltyActive,formBonus)=>{
    const diff=DIFFICULTIES.find(d=>d.key===quest.difficulty);
    let xp=diff.xp;
  ...
  const completeQuest=useCallback((questId,rect)=>{
    ...
    const formBonus=calcFormationBonus(state.shadowArmy);
    const penaltyActive=state.penaltyZone?.active;
    const xpGain=computeXpGain(quest,streakBonusPct,equipBonuses,skillBonuses,penaltyActive,formBonus);
    const diff=DIFFICULTIES.find(d=>d.key===quest.difficulty);
    const typeCfg=QUEST_TYPES_CONFIG[quest.type]||QUEST_TYPES_CONFIG.side;
  ...
  const finishDungeon=useCallback((dungeon,result)=>{
    ...
    if(didLevelUp){setPrevRank(oldRank);setLevelUp(newLevel);}
    else notify(result.won?`${dungeon.name} bezwungen! +${result.xp} XP · ${result.floorsCleared||"?"}/${dungeon.floors} Floors`:`Niederlage in ${dungeon.name}.`,result.won?"dungeon":"defeat");
  },[state,persist,processAchievements,notify]);
  ...
  const formationBonus=calcFormationBonus(shadowArmy);
